...
<img src="web/public/logo.png" alt="Aether OS Logo" width="128" height="128" style="border-radius: 28px; box-shadow: 0 20px 40px -15px rgba(59, 130, 246, 0.5);" />
...
[![Groq](https://img.shields.io/badge/Inference-Groq%20Llama%203-F55036?style=for-the-badge)](https://groq.com)
...
```mermaid
flowchart TD
    User([Learner Ingestion]) --> IngestionAgent[1. Ingestion Agent\nGemini 3.6 Flash]
    IngestionAgent --> ConceptArchitect[2. Concept Architect\nGroq Llama 3 120B]
    ConceptArchitect --> LearningDNA[3. Learning DNA Agent\nGroq Llama 3 120B]

    subgraph ParallelFanout ["⚡ Parallel Generator Fan-Out (sub-15s Latency)"]
        LearningDNA --> ContentForge[4. Content Forge\nGroq Llama 3 120B]
        LearningDNA --> QuizMaster[5. Quiz Master\nGroq Llama 3 20B]
        LearningDNA --> FlashcardSmith[6. Flashcard Smith\nGroq Llama 3 120B]
    end

    ContentForge & QuizMaster & FlashcardSmith --> StudyHub[(Supabase pgvector DB)]

    subgraph AdaptiveEvaluationLoop ["🔄 Adaptive Mastery & Rescue Loop"]
        StudentAnswer([Student Quiz Answer]) --> Grader[7. Semantic Grader\nGroq Llama 3 20B]
        Grader -->|Grade Passed q >= 3| Scheduler[9. SM-2 Scheduler\nMathematical Engine]
        Grader -->|Grade Failed q < 3| RemediationCoach[8. Remediation Coach\n6-Step Strategy Ladder]
        RemediationCoach -->|Micro-Check Passed| Scheduler
    end

    subgraph ConversationalLayer ["💬 Socratic RAG Layer"]
        ChatQuery([Student Chat Question]) --> ChatTutor[10. Chat Tutor\nGroq 120B + pgvector RAG]
    end

    Scheduler --> MemoryTelemetry([Analyzer Telemetry Dashboard])
```
